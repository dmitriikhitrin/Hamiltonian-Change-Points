...
class MyStateVector:
    ...
    def measure_in_basis(self, basis, qubits):
        ...
        transformed_sv = self.sv.copy().evolve(qc.inverse())
    ...
    def measure_in_dt_basis(self, lab, x=''):
        '''
        returns projectors corresponding to DT basis measurement outcome, where |hyp> is self
        ...
        for t in range(n-k):
            ...
            lab_x_t = lab_x_t.get_conditioned_state([np.eye(2)] + projectors)
...
def certify(hyp, lab):
    ...
    if k == n: # DT basis is empty
        hyp_prime = hyp.get_conditioned_state(project_x + [np.eye(2)])
        lab_prime = lab.get_conditioned_state(project_x + [np.eye(2)])
    else:
        l_projectors = hyp.measure_in_dt_basis(lab, x)
